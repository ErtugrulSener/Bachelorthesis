...
exports.hash = async function(password, salt) {
  const pepperedSalt = defaults.pepper.concat(salt);
  const hash = (await pbkdf2(password, pepperedSalt, defaults.iterations, defaults.keyLength, defaults.digest)).toString();
  return { salt, hash };
};

exports.compare = async function(password, { salt, hash }) {
  const pepperedSalt = defaults.pepper.concat(salt);
  const comparisonHash = (await pbkdf2(password, pepperedSalt, defaults.iterations, defaults.keyLength, defaults.digest)).toString();
